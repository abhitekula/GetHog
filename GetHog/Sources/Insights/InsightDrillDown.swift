...
struct InsightDrillContext {
    ...
    let axis: InsightDrillAxis
    let open: (InsightDrillRequest) -> Void

    init(source: JSONValue, axis: InsightDrillAxis, open: @escaping (InsightDrillRequest) -> Void) {
        self.source = source
        self.axis = axis
        self.open = open
    }
    ...
    init?(insight: Insight, open: @escaping (InsightDrillRequest) -> Void) {
...
extension EnvironmentValues {
    @Entry var insightDrill: InsightDrillContext?
}
...
/// did not ask for.
struct InsightDrillRequest: Identifiable, Hashable {
    let selected: InsightDrill
    /// Every drill on the same axis, including `selected`. One entry means no
    /// picker is drawn.
    let siblings: [InsightDrill]
    /// What the picker is picking, in the reader's words — "Step", "Week".
    let axisLabel: String

    var id: InsightDrillKind { selected.kind }

    init(selected: InsightDrill, siblings: [InsightDrill] = [], axisLabel: String) {
