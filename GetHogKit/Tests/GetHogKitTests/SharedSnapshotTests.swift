...
struct SharedSnapshotTests {
    ...
    // MARK: - The platform matrix
    //
    // Three new shells — watch, TV and vision — read the same container the
    // phone writes. Each gets its own test rather than one loop, because the
    // thing being pinned is a *per-platform* claim and a loop would report a
    // single failure for whichever platform broke first.
    //
    // These pin behavior that is already correct: `platformRequiresTeamIDPrefix`
    // is `#if os(macOS)` / `#else`, so every non-Mac platform already resolves
    // the unprefixed spelling. That is the finding, and it is worth a test
    // precisely because it is a default — a future `#elseif os(watchOS)` added
    // for some unrelated reason would silently rename the watch's container and
    // nothing else in the suite would notice.

    @Test("watchOS wears the iOS spelling — no team prefix")
    func watchOSIdentifier() {
        // The platform-independent rule the watch app compiles against, pinned
        // from whichever platform runs the suite.
        #expect(SharedSnapshotStore.resolvedAppGroupIdentifier(
            teamIDPrefix: "EXAMPLETEAM.", requiresTeamIDPrefix: false
        ) == "group.app.gethog")
        #expect(SharedSnapshotStore.resolvedAppGroupIdentifier(
        ...
        // And the live branch, if this suite is ever run on the wrist.
        #if os(watchOS)
        #expect(SharedSnapshotStore.platformRequiresTeamIDPrefix == false)
        #expect(SharedSnapshotStore.appGroupIdentifier(teamIDPrefix: "EXAMPLETEAM.")
            == "group.app.gethog")
    ...
    @Test("tvOS wears the iOS spelling — no team prefix")
    func tvOSIdentifier() {
        #expect(SharedSnapshotStore.resolvedAppGroupIdentifier(
            teamIDPrefix: "EXAMPLETEAM.", requiresTeamIDPrefix: false
        ) == "group.app.gethog")
    ...
    @Test("visionOS wears the iOS spelling — no team prefix")
    func visionOSIdentifier() {
        #expect(SharedSnapshotStore.resolvedAppGroupIdentifier(
            teamIDPrefix: "EXAMPLETEAM.", requiresTeamIDPrefix: false
        ) == "group.app.gethog")
        #expect(SharedSnapshotStore.resolvedAppGroupIdentifier(
